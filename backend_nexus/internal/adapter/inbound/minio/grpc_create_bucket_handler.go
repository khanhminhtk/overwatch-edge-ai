package minio

import (
	"context"

	"backend_nexus/internal/utils"
	pb "backend_nexus/proto"
)

func (h *MinioGRPCServerHandler) CreateBucket(ctx context.Context, req *pb.CreateBucketRequest) (*pb.CreateBucketResponse, error) {
	resp := &pb.CreateBucketResponse{}
	if req != nil {
		resp.RequestId = req.GetRequestId()
	}

	if req == nil || req.GetRequestId() == "" || req.GetBucketName() == "" {
		resp.Success = false
		resp.Code = utils.CodeInvalidArgument
		resp.Message = "request_id and bucket_name are required"
		return resp, nil
	}

	if ctx.Err() == context.Canceled {
		if h.Logger.Logger != nil {
			h.Logger.Info("CreateBucket: request canceled by client")
		}
		resp.Success = false
		resp.Code = utils.CodeRequestCanceled
		resp.Message = "request canceled by client"
		return resp, nil
	}

	if err := h.Minio.CreateBucket(ctx, req.GetBucketName()); err != nil {
		if h.Logger.Logger != nil {
			h.Logger.Error("CreateBucket: error occurred while creating bucket", "error", err)
		}
		resp.Success = false
		resp.Code = utils.CodeInternalError
		resp.Message = "error occurred while creating bucket"
		return resp, nil
	}

	resp.Success = true
	resp.Code = utils.CodeOK
	resp.Message = "bucket created"
	return resp, nil
}
