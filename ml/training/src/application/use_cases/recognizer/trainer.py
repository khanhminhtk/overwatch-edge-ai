...
class RecognizerTrainer:
    ...
    def _validate_training_config(self) -> None:
        ...
        eval_every_n_epochs = int(loop_cfg.get("eval_every_n_epochs", 1))
        if eval_every_n_epochs <= 0:
            raise ValueError(
                f"RecognizerTrainer: loop.eval_every_n_epochs must be > 0, got {eval_every_n_epochs}"
            )
    ...
    def _run_epoch(
        ...
        for batch_idx, batch in enumerate(progress):
            with torch.set_grad_enabled(training):
                autocast_ctx = (
                    torch.amp.autocast(device_type="cuda", enabled=amp_enabled, dtype=amp_dtype)
    ...
    def train(self) -> dict[str, object]:
        ...
        fast_mode = bool(hooks_cfg.get("fast_mode", False))
        loop_cfg = cfg["loop"]
        loss_cfg = cfg["loss"]
        ...
        eval_every_n_epochs = max(1, int(loop_cfg.get("eval_every_n_epochs", 1)))
        ...
        with ExitStack() as stack:
            tb_log_every_n_steps = int(tensortboard_cfg.get("log_every_n_steps", 1))
            tb_log_lr_every_n_steps = int(tensortboard_cfg.get("log_lr_every_n_steps", 0))
            tb_flush_every_n_steps = int(tensortboard_cfg.get("flush_every_n_steps", 10))
            tb_hist_every_n_steps = int(tensortboard_cfg.get("log_histograms_every_n_steps", 0))
            tb_max_modules = int(tensortboard_cfg.get("max_modules", 100))
            tb_leaf_only = bool(tensortboard_cfg.get("leaf_only", True))
            if fast_mode:
                tb_log_every_n_steps = max(100, tb_log_every_n_steps)
                tb_log_lr_every_n_steps = max(100, tb_log_lr_every_n_steps)
                tb_flush_every_n_steps = max(500, tb_flush_every_n_steps)
                tb_hist_every_n_steps = 0
                tb_max_modules = min(tb_max_modules, 20)
                tb_leaf_only = True
            ...
            tb_hook = stack.enter_context(
                TensorBoardScalarHook(
                    ...
                    module_names=None,
                    log_every_n_steps=tb_log_every_n_steps,
                    leaf_only=tb_leaf_only,
                    log_histograms_every_n_steps=tb_hist_every_n_steps,
                    flush_every_n_steps=tb_flush_every_n_steps,
                    max_modules=tb_max_modules,
                )
            )
            nan_guard_enabled = bool(hooks_cfg["nan_guard"]["enabled"]) and not fast_mode
            ...
            for epoch_idx in range(start_epoch_idx, epochs):
                ...
                train_metrics = self._run_epoch(
                    ...
                    tb_hook=tb_hook,
                    log_prefix="train",
                    log_lr_every_n_steps=tb_log_lr_every_n_steps,
                ...
                should_eval = (epoch_num % eval_every_n_epochs == 0) or (epoch_num == epochs)
                if should_eval:
                ...
                if should_eval:
                    ...
                    LOGGER.info(
                        "Epoch %s/%s - Train Loss: %.4f (skip val; eval_every_n_epochs=%s)",
                        epoch_num,
                        epochs,
                        train_metrics["loss"],
                        eval_every_n_epochs,
