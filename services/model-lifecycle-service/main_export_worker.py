...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run export worker for queued jobs")
    ...
    parser.add_argument(
        "--event-topic",
        default=os.getenv("EXPORT_EVENT_TOPIC", "export.events"),
    )
    parser.add_argument(
        "--training-mode",
        default=os.getenv("EXPORT_MODE", "local"),
    )
    parser.add_argument(
        "--idle-sleep-seconds",
        type=float,
        default=float(os.getenv("EXPORT_IDLE_SLEEP_SECONDS", "2.0")),
    )
    parser.add_argument(
        "--log-level",
