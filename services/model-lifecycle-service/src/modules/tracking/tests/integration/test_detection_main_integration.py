...
class DetectionMainIntegrationTest(unittest.TestCase):
    def test_main_runs_detection_workflow(self) -> None:
        config_yaml = SERVICE_ROOT / "config/model_lifecycle_orchestrator_config.yaml"
        config_env = SERVICE_ROOT / "config/.env"
        script_path = SERVICE_ROOT / "src/modules/tracking/tests/integration/detection_main.py"
