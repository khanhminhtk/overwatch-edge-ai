...
from src.modules.training.application.use_case.train_recognizer import TrainRecognizerUseCase
from src.modules.training.domain.value_objects.training_spec import TrainingSpec
from src.platform.logger import Logger


class TrainRecognizerUseCaseUnitTest(unittest.TestCase):
    def setUp(self) -> None:
        self.logger = MagicMock(spec=Logger)
        self.use_case = TrainRecognizerUseCase(logger=self.logger)
    ...
    def test_execute_runs_script_with_correct_args(self) -> None:
        ...
        self.assertIn("train_recognizer.sh", str(cmd[1]))
        self.assertIn("--mode", cmd)
        self.assertIn("dev", cmd)
        self.assertIn("--dataset_version", cmd)
        self.assertIn("v1", cmd)
    ...
    def test_execute_raises_on_nonzero_exit(self) -> None:
        ...
        with patch("subprocess.Popen", return_value=mock_process) as mock_popen:
            ...
            self.assertIn("Recognizer training failed", str(ctx.exception))
