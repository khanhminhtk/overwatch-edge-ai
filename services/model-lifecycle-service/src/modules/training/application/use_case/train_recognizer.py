...
class TrainRecognizerUseCase:
    ...
    def execute(self, spec: TrainingSpec) -> TrainingResult:
        ...
        script_path = project_root / "ml" / "training" / "scripts" / "train_recognizer.sh"
        ...
        for line in process.stdout:
            self._logger.info("[TRAIN_RECOGNIZER_OUT]", line.rstrip())
        process.wait()

        if process.returncode != 0:
            raise RuntimeError(
                f"Recognizer training failed with exit code {process.returncode}"
            )

        duration_ms = (time.perf_counter() - started_at) * 1000
        self._logger.info(
            "[TRAIN_RECOGNIZER_SUCCEEDED]",
