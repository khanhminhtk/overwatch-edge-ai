...
from src.modules.exports.application.use_case import ExportDetectionUseCase
...
def build_use_case() -> ExportDetectionUseCase:
    ...
    return ExportDetectionUseCase(
        ...
        logger=Logger("ExportDetectionUseCase"),
    )


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export detection (YOLO) .pt checkpoint to ONNX"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
...
def main() -> None:
    ...
    detection_config = use_case._detection_config
    if args.checkpoint is None:
        checkpoint_path = (
            Path(detection_config.checkpoint_dir)
    ...
    print(f"Detection ONNX exported: {result.output_path}")
