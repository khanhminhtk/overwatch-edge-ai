...
def resolve_placeholders(data: Any, env: dict[str, str], path: str = "") -> Any:
    ...
    if isinstance(data, list):
        return [resolve_placeholders(item, env, f"{path}[{index}]") for index, item in enumerate(data)]
    if isinstance(data, tuple):
        return tuple(resolve_placeholders(item, env, f"{path}[{index}]") for index, item in enumerate(data))
...
def cast_value(value: Any, target_type: Any, path: str = "") -> Any:
    ...
    if origin is list:
        ...
        return [cast_value(item, item_type, f"{path}[{index}]") for index, item in enumerate(value)]
    ...
    if origin is tuple:
        ...
        if len(item_types) == 2 and item_types[1] is Ellipsis:
            return tuple(cast_value(item, item_types[0], f"{path}[{index}]") for index, item in enumerate(items))
