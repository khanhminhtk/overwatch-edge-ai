...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Kafka consumer for training job ingestion")
    ...
    parser.add_argument(
        "--dlq-topic",
        default=os.getenv("TRAINING_DLQ_TOPIC", "training.dlq"),
    )
    parser.add_argument(
        "--log-level",
...
def create_consumer_runtime(
    *,
    ...
    training_job_repository = TrainingJobRepository(sql_handler=sql_handler, logger=logger)
    export_job_repository = ExportJobRepository(sql_handler=sql_handler, logger=logger)

    topic_handlers: list[tuple[str, Any]] = []
    for job_key in job_keys:
        job_config = kafka_config.jobs[job_key]
        if job_key.startswith("train_"):
            handler = KafkaCommandHandler(
                job_repository=training_job_repository,
                producer=producer,
                logger=logger,
        ...
        elif job_key.startswith("export_"):
            handler = ExportKafkaCommandHandler(
                job_repository=export_job_repository,
                producer=producer,
                logger=logger,
                consumer_group=job_config.group_id,
                server_id=server_id,
            ...
            raise ValueError(f"Unsupported job_key handler mapping: {job_key}")
        topic_handlers.append((job_config.topic, handler))

    handler = TopicRoutingKafkaHandler.from_topic_handlers(topic_handlers)
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    job_keys = args.job_keys or [os.getenv("TRAINING_CONSUMER_JOB_KEY", "train_recognizer")]
